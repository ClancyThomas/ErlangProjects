...
cone_combinations(nil,_) -> fail;
cone_combinations([],[]) -> [];
cone_combinations(Top_flavors,Bottom_flavors) -> 
	[{First, Second} || First<-Top_flavors, Second<-Bottom_flavors].


cone_combinations(nil) -> fail;
cone_combinations([]) -> [];
cone_combinations(Flavor_list)-> [{First, Second} || First<-Flavor_list, Second<-Flavor_list].


most_popular_combinations(Count, []) when Count == 0 -> [];
most_popular_combinations(Count,List) when Count >= 0, Count =< length(List) -> lists:sublist(List, Count);
most_popular_combinations(_,_) -> fail.
