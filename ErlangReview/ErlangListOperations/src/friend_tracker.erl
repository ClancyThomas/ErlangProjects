...
run(Friend_list) ->
	receive
		{Pid,{add,List}} -> 
			...
			Pid ! received;
		{Pid,{has_friend,Name}} -> 
			Pid ! lists:member(Name, Friend_list);
		{Pid,{has_friends,Name}} -> 
			Remaining = lists:filter(fun (Elem) -> lists:member(Elem, Friend_list) end, Name),
			Pid ! length(Remaining) == length(Name);
		{Pid,{remove,Name}} -> 
			lists:delete(Name, Friend_list),
			Pid ! received;
		{Pid,get} ->
			Pid ! Friend_list;
        {Pid,_other} ->
