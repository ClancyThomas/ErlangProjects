...
-export([init/1]).
%%API functions
-export([start/0,start/1,start/3,add_child/3,remove_child/2,generate_spec_list/2]).
...
%% @end
%%--------------------------------------------------------------------
add_child(Supervisor_name,Child_name,Child_module)->
        supervisor:start_child(Supervisor_name,generate_spec(Child_module,Child_name)).
...
init(Start_info) ->
    ...
    %ChildSpecList = [generate_spec(word_server, one), generate_spec(word_server, two)],
    ChildSpecList = generate_spec_list(word_server,10),
...
generate_spec_list(_,0) -> [];
generate_spec_list(Module,Number) ->
    [generate_spec(Module,list_to_atom(lists:flatten(io_lib:format("~p_~p", [Module, Number]))))]++generate_spec_list(Module,Number-1).
